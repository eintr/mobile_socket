...
decode(v1, Packet, Context) ->
	...
	FrameBody =  mycrypt:decrypt_frame_body(CryptedFrameBody, Encrypt, Zip, kv:get(sharedkey, Context)),
	case FrameBody of
		...
		<<1:1/big-integer, ?OP_SOCKET_KEY_SYNC:15/big-integer, CRC32:32/big-integer, SharedKey/binary>> ->
			{ctl, socket, key_sync, {CRC32, mycrypt:decrypt_shared_key(SharedKey, kv:get(privkey, Context))}};
...
encode(v1, {data, FlowID, CryptFlag, Zip, RawData}, Context) ->
	RawBody = <<	0:1/big-integer,
					...
					RawData/binary >>,
	CryptedBody =  mycrypt:encrypt_frame_body(RawBody, CryptFlag, Zip, kv:get(sharedkey, Context)),
...
encode(v1, {ctl, pipeline, open, {FlowID, CryptFlag, Zip, MaxDelay, ReplyFlags, Data}}, Context) ->
	Rawbody = <<	1:1/big-integer,
					...
					Data/binary>>,
	Frame_body = mycrypt:encrypt_frame_body(Rawbody, CryptFlag, Zip, kv:get(sharedkey, Context)),
...
encode(v1, {ctl, pipeline, close, {FlowID, CryptFlag, Zip}}, Context) ->
	...
	Frame_body = mycrypt:encrypt_frame_body(Raw_body, CryptFlag, Zip, kv:get(sharedkey, Context)),
	{ok, <<1:3/big-integer, CryptFlag:2/big-integer, Zip:1/big-integer, 0:2/big-integer, Frame_body/binary>>};
